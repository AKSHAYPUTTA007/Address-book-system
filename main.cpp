...
public:
  ...
  void searchContact() {
    ...
    if (opt == 1) {
      ...
      if (found == 0)
        cout << "  No match found for: " << query << "\n";
      else
        cout << "  " << found << " contact(s) found.\n";
  ...
  void updateContact() {
    ...
    getline(cin, val);
    if (!val.empty())
      contacts[idx].name = val;
    cout << "  Phone    [" << contacts[idx].phone << "]: ";
    getline(cin, val);
    if (!val.empty()) {
      if (val != phone && findByPhone(val) != -1)
        cout << "  Phone taken. Keeping old.\n";
      else
        contacts[idx].phone = val;
    }
    cout << "  Email    [" << contacts[idx].email << "]: ";
    getline(cin, val);
    if (!val.empty())
      contacts[idx].email = val;
    cout << "  Address  [" << contacts[idx].address << "]: ";
    getline(cin, val);
    if (!val.empty())
      contacts[idx].address = val;
    cout << "  Category [" << contacts[idx].category << "]: ";
    getline(cin, val);
    if (!val.empty())
      contacts[idx].category = val;
  ...
  void deleteContact() {
    ...
    if (c == 'y' || c == 'Y') {
      contacts.erase(contacts.begin() +
                     idx); // vector removes + shifts automatically
